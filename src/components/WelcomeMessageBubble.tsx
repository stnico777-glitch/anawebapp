...
export default function WelcomeMessageBubble() {
  ...
  const { showTyping, typingFading, visibleMessages } = useChatSequence({
    ...
    /** Close/back clears timers; reopen runs the full sequence (avoids stuck typing if user backs out mid-load). */
  ...
  return createPortal(
    <div
      className={`welcome-bubble-backdrop fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-3 sm:p-6 ${
