...
export default function AudioCollectionForm({
  ...
  }, [card?.id, card?.title, card?.metaLine, card?.imageUrl, card?.summary, card?.linkHref, card?.sortOrder]);
  ...
  async function handleDelete() {
    if (!card || !confirm("Delete this collection card?")) return;
  ...
  return (
    <>
      ...
      >
        {triggerLabel ?? (card ? "Edit" : "Add collection")}
      </button>
      {open && (
