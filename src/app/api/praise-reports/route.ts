...
export async function POST(request: Request) {
  ...
  if (!author) {
    return NextResponse.json({ error: "Sign in to post a praise." }, { status: 401 });
  }

  /** Per-user burst cap: 5 posts per minute, 30 per hour. */
  const minute = rateLimit(`post:praise:user:${author.userId}:1m`, 5, 60_000);
  if (!minute.ok) {
  ...
  const hour = rateLimit(`post:praise:user:${author.userId}:1h`, 30, 60 * 60_000);
  if (!hour.ok) {
    return NextResponse.json(
